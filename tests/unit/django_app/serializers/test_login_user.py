...
from django_auth.core.use_cases.login_user import dto as login_dto
from django_auth.core.use_cases.login_user import types
from django_auth.django_app import serializers


class TestLoginUserSerializer:
    def test_serialize(
        self,
        login_user_payload: types.LoginUserPayload,
        login_user_request: Request,
        authenticator: Callable[[Request, Any], AbstractBaseUser | None],
    ) -> None:
        assert serializers.LoginUserSerializer(
    ...
    def test_invalid_payload_serialize(
        ...
        assert not serializers.LoginUserSerializer(
            authenticator=authenticator,
    ...
    def test_to_dto(
        ...
        assert isinstance(dto, login_dto.LoginUserDTO)
